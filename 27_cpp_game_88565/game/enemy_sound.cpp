...
FOnShooterCharacterWeaponChange Enemy::NotifyWeaponChange;

Enemy::Enemy(const FObjectInitializer& ObjectInitializer)
...
void Enemy::PostInitializeComponents()
{
...
void Enemy::PossessedBy(class AController* InController)
{
...
FRotator Enemy::GetAimOffsets() const
{
...
bool Enemy::IsEnemyFor(AController* TestPC) const
{
...
void Enemy::UpdateTeamColors(UMaterialInstanceDynamic* UseMID)
{
	if (UseMID)
	{
		AShooterPlayerState* MyPlayerState = Cast<AShooterPlayerState>(PlayerState);
...
void Enemy::OnCameraUpdate(const FVector& CameraLocation, const FRotator& CameraRotation)
{
...
void Enemy::FellOutOfWorld(const class UDamageType& dmgType)
{
...
void Enemy::KilledBy(APawn* EventInstigator)
{
...
float Enemy::TakeDamage(float Damage, struct FDamageEvent const& DamageEvent, class AController* EventInstigator, class AActor* DamageCauser)
{
...
bool Enemy::CanDie(float KillingDamage, FDamageEvent const& DamageEvent, AController* Killer, AActor* DamageCauser) const
{
...
bool Enemy::Die(float KillingDamage, FDamageEvent const& DamageEvent, AController* Killer, AActor* DamageCauser)
{
	...
	NetUpdateFrequency = GetDefault<Enemy>()->NetUpdateFrequency;
...
void Enemy::OnDeath(float KillingDamage, struct FDamageEvent const& DamageEvent, class APawn* PawnInstigator, class AActor* DamageCauser)
{
	if (bIsDying)
	{
		return;
	...
	if (DeathAnimDuration > 0.f)
	{
		...
		GetWorldTimerManager().SetTimer(TimerHandle, this, &Enemy::SetRagdollPhysics, FMath::Max(0.1f, TriggerRagdollTime), false);
	}
	else
	{
		SetRagdollPhysics();
...
void Enemy::PlayHit(float DamageTaken, struct FDamageEvent const& DamageEvent, class APawn* PawnInstigator, class AActor* DamageCauser)
{
	if (Role == ROLE_Authority)
	{
...
void Enemy::ReplicateHit(float Damage, struct FDamageEvent const& DamageEvent, class APawn* PawnInstigator, class AActor* DamageCauser, bool bKilled)
{
	...
	LastTakeHitInfo.PawnInstigator = Cast<Enemy>(PawnInstigator);
...
void Enemy::OnRep_LastTakeHitInfo()
{
...
void Enemy::SpawnDefaultInventory()
{
	if (Role < ROLE_Authority)
	{
		return;
...
void Enemy::AddWeapon(AShooterWeapon* Weapon)
{
...
void Enemy::RemoveWeapon(AShooterWeapon* Weapon)
{
	if (Weapon && Role == ROLE_Authority)
	{
...
AShooterWeapon* Enemy::FindWeapon(TSubclassOf<AShooterWeapon> WeaponClass)
{
...
void Enemy::EquipWeapon(AShooterWeapon* Weapon)
{
	if (Weapon)
	{
		if (Role == ROLE_Authority)
...
bool Enemy::ServerEquipWeapon_Validate(AShooterWeapon* Weapon)
{
	return true;
}

void Enemy::ServerEquipWeapon_Implementation(AShooterWeapon* Weapon)
{
	EquipWeapon(Weapon);
}

void Enemy::OnRep_CurrentWeapon(AShooterWeapon* LastWeapon)
{
	SetCurrentWeapon(CurrentWeapon, LastWeapon);
}

void Enemy::SetCurrentWeapon(AShooterWeapon* NewWeapon, AShooterWeapon* LastWeapon)
{
...
bool Enemy::CanReload() const
{
	return true;
}

void Enemy::SetTargeting(bool bNewTargeting)
{
...
bool Enemy::ServerSetTargeting_Validate(bool bNewTargeting)
{
	return true;
}

void Enemy::ServerSetTargeting_Implementation(bool bNewTargeting)
{
	SetTargeting(bNewTargeting);
}


void Enemy::SetRunning(bool bNewRunning, bool bToggle)
{
...
bool Enemy::ServerSetRunning_Validate(bool bNewRunning, bool bToggle)
{
	return true;
}

void Enemy::ServerSetRunning_Implementation(bool bNewRunning, bool bToggle)
{
	SetRunning(bNewRunning, bToggle);
}

void Enemy::UpdateRunSounds()
{
...
float Enemy::PlayAnimMontage(class UAnimMontage* AnimMontage, float InPlayRate, FName StartSectionName)
{
...
void Enemy::StopAnimMontage(class UAnimMontage* AnimMontage)
{
	USkeletalMeshComponent* UseMesh = GetPawnMesh();
	if (AnimMontage && UseMesh && UseMesh->AnimScriptInstance &&
...
void Enemy::StopAllAnimMontages()
{
	USkeletalMeshComponent* UseMesh = GetPawnMesh();
...
void Enemy::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	check(PlayerInputComponent);

	PlayerInputComponent->BindAxis("MoveForward", this, &Enemy::MoveForward);
	PlayerInputComponent->BindAxis("MoveRight", this, &Enemy::MoveRight);
	PlayerInputComponent->BindAxis("MoveUp", this, &Enemy::MoveUp);
	PlayerInputComponent->BindAxis("Turn", this, &APawn::AddControllerYawInput);
	PlayerInputComponent->BindAxis("TurnRate", this, &Enemy::TurnAtRate);
	PlayerInputComponent->BindAxis("LookUp", this, &APawn::AddControllerPitchInput);
	PlayerInputComponent->BindAxis("LookUpRate", this, &Enemy::LookUpAtRate);

	PlayerInputComponent->BindAction("Fire", IE_Pressed, this, &Enemy::OnStartFire);
	PlayerInputComponent->BindAction("Fire", IE_Released, this, &Enemy::OnStopFire);

	PlayerInputComponent->BindAction("Targeting", IE_Pressed, this, &Enemy::OnStartTargeting);
	PlayerInputComponent->BindAction("Targeting", IE_Released, this, &Enemy::OnStopTargeting);

	PlayerInputComponent->BindAction("NextWeapon", IE_Pressed, this, &Enemy::OnNextWeapon);
	PlayerInputComponent->BindAction("PrevWeapon", IE_Pressed, this, &Enemy::OnPrevWeapon);

	PlayerInputComponent->BindAction("Reload", IE_Pressed, this, &Enemy::OnReload);

	PlayerInputComponent->BindAction("Jump", IE_Pressed, this, &Enemy::OnStartJump);
	PlayerInputComponent->BindAction("Jump", IE_Released, this, &Enemy::OnStopJump);

	PlayerInputComponent->BindAction("Run", IE_Pressed, this, &Enemy::OnStartRunning);
	PlayerInputComponent->BindAction("RunToggle", IE_Pressed, this, &Enemy::OnStartRunningToggle);
	PlayerInputComponent->BindAction("Run", IE_Released, this, &Enemy::OnStopRunning);
}


void Enemy::MoveForward(float Val)
{
...
void Enemy::MoveRight(float Val)
{
	if (Val != 0.f)
	{
...
void Enemy::TurnAtRate(float Val)
{
	AddControllerYawInput(Val * BaseTurnRate * GetWorld()->GetDeltaSeconds());
}

void Enemy::LookUpAtRate(float Val)
{
	AddControllerPitchInput(Val * BaseLookUpRate * GetWorld()->GetDeltaSeconds());
}

void Enemy::OnStartFire()
{
	AShooterPlayerController* MyPC = Cast<AShooterPlayerController>(Controller);
...
void Enemy::OnStopFire()
{
	StopWeaponFire();
}

void Enemy::OnStartTargeting()
{
	AShooterPlayerController* MyPC = Cast<AShooterPlayerController>(Controller);
	if (MyPC && MyPC->IsGameInputAllowed())
	{
...
void Enemy::OnStopTargeting()
{
	SetTargeting(false);
}

void Enemy::OnNextWeapon()
{
	AShooterPlayerController* MyPC = Cast<AShooterPlayerController>(Controller);
	if (MyPC && MyPC->IsGameInputAllowed())
	{
...
void Enemy::OnStartRunningToggle()
{
	AShooterPlayerController* MyPC = Cast<AShooterPlayerController>(Controller);
	if (MyPC && MyPC->IsGameInputAllowed())
	{
...
void Enemy::OnStopRunning()
{
	SetRunning(false, false);
}

bool Enemy::IsRunning() const
{
...
void Enemy::OnStopJump()
{
	bPressedJump = false;
	StopJumping();
}

void Enemy::PreReplication(IRepChangedPropertyTracker & ChangedPropertyTracker)
{
	Super::PreReplication(ChangedPropertyTracker);

	DOREPLIFETIME_ACTIVE_OVERRIDE(Enemy, LastTakeHitInfo, GetWorld() && GetWorld()->GetTimeSeconds() < LastTakeHitTimeTimeout);
}
...
void Enemy::GetLifetimeReplicatedProps(TArray< FLifetimeProperty > & OutLifetimeProps) const
{
	...
	DOREPLIFETIME_CONDITION(Enemy, Inventory, COND_OwnerOnly);

	DOREPLIFETIME_CONDITION(Enemy, bIsTargeting, COND_SkipOwner);
	DOREPLIFETIME_CONDITION(Enemy, bWantsToRun, COND_SkipOwner);

	DOREPLIFETIME_CONDITION(Enemy, LastTakeHitInfo, COND_Custom);

	DOREPLIFETIME(Enemy, CurrentWeapon);
	DOREPLIFETIME(Enemy, Health);
}

bool Enemy::IsReplicationPausedForConnection(const FNetViewer& ConnectionOwnerNetViewer)
{
...
void Enemy::OnReplicationPausedChanged(bool bIsReplicationPaused)
{
	GetMesh()->SetHiddenInGame(bIsReplicationPaused, true);
}

AShooterWeapon* Enemy::GetWeapon() const
{
	return CurrentWeapon;
}

int32 Enemy::GetInventoryCount() const
{
	return Inventory.Num();
}

AShooterWeapon* Enemy::GetInventoryWeapon(int32 index) const
{
	return Inventory[index];
}

USkeletalMeshComponent* Enemy::GetPawnMesh() const
{
	return IsFirstPerson() ? Mesh1P : GetMesh();
}

USkeletalMeshComponent* Enemy::GetSpecifcPawnMesh(bool WantFirstPerson) const
{
	return WantFirstPerson == true ? Mesh1P : GetMesh();
}

FName Enemy::GetWeaponAttachPoint() const
{
	return WeaponAttachPoint;
}

float Enemy::GetTargetingSpeedModifier() const
{
	return TargetingSpeedModifier;
}

bool Enemy::IsTargeting() const
{
	return bIsTargeting;
}

float Enemy::GetRunningSpeedModifier() const
{
...
bool Enemy::IsFirstPerson() const
{
	return IsAlive() && Controller && Controller->IsLocalPlayerController();
}

int32 Enemy::GetMaxHealth() const
{
	return GetClass()->GetDefaultObject<Enemy>()->Health;
}
...
float Enemy::GetLowHealthPercentage() const
{
	return LowHealthPercentage;
}

void Enemy::UpdateTeamColorsAllMIDs()
{
...
void Enemy::BuildPauseReplicationCheckPoints(TArray<FVector>& RelevancyCheckPoints)
{
